...
import com.wellsfargo.bankapp.service.TransactionService;abstract
...
public class TransactionControllerTest {
	...
	@Test
	public void testsendAmount() throws Exception {
		Long senderAccId = 1L;
        Long receiverAccId = 2L;
        double amount = 200;
        
        JSONObject obj = new JSONObject();
        obj.put("senderAccId", senderAccId);
        obj.put("receiverAccId", receiverAccId);
        obj.put(amount, 200);
		String json = mapper.writeValueAsString(obj);
		Mockito.when(transactionService.addTransaction(senderAccId,receiverAccId,amount)).thenReturn("Transaction Succesful!");
		mvc.perform(post("api/v1/transaction/send").header("Authorization", "Bearer "+ jwt_token)
